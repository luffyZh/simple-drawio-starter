...
(function()
{
	 ...
	 * 
	 */
	EditorUi.prototype.createImageDataUri = function(canvas, xml, format, dpi, scale = 0.92)
	{
		var data = canvas.toDataURL('image/' + format, scale);
...
//		}
	...
	EditorUi.prototype.getCurrentFileThumbnail = function(scale) {
		const cells = this.editor.graph.model.root;
		const xml = this.editor.getGraphXml();
		this.editor.exportToCanvas(mxUtils.bind(this, function(canvas)
			{
				try
				{
					window.thumbnailImage = this.createImageDataUri(canvas, xml, 'png', null, scale);
				}
				catch (e)
				{
					this.handleError(e);
	 ...
	 */
	EditorUi.prototype.copyImage = function(cells, xml, scale, mode)
	{
		try
		{
			if (navigator.clipboard != null && this.spinner.spin(document.body, mxResources.get('exporting')))
			{
				this.editor.exportToCanvas(mxUtils.bind(this, function(canvas, svgRoot)
				{
					try
					{
						...
						dataUrl = this.createImageDataUri(canvas, xml, 'png');
						/**
						 * 单独处理
						 */
						mode === App.MODE_MUSIC && this.music.setThumbnail(dataUrl);
						var w = parseInt(svgRoot.getAttribute('width'));
						var h = parseInt(svgRoot.getAttribute('height'));
