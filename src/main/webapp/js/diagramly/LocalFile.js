...
/**
 * 开发过程中使用 api
    const xmlData = this.ui.editor.getGraphXml();
		const prettyXmlStr = mxUtils.getPrettyXml(xmlData);
		console.log(this.ui.editor.graph.model.root, this.ui.editor.getGraphXml());
	  // 第四个参数传入云音乐，就会获取到缩略图的 base64
	  this.ui.copyImage(this.ui.editor.graph.model.root, this.ui.editor.getGraphXml(), .5, 'music');
 */
