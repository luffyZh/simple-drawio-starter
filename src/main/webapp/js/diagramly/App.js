		...
		if (urlParams['dev'] == '1' && CryptoJS != null && App.mode != App.MODE_DROPBOX && App.mode != App.MODE_TRELLO)
		{
			...
			if (scripts != null && scripts.length > 0)
			{
				...
				if (CryptoJS.MD5(content).toString() != 'b02227617087e21bd49f2faa15164112')
				{
					...
					console.warn('[Dev] Bootstrap script change requires update of CSP');
				}
			}
...
App.prototype.init = function()
{
	...
	/**
	 * Creates netease cloud music client.
	 */
		 this.music = (!mxClient.IS_IE || document.documentMode == 10 ||
			mxClient.IS_IE11 || mxClient.IS_EDGE) &&
			(urlParams['gh'] != '0' && (urlParams['embed'] != '1' ||
			urlParams['gh'] == '1')) ? new NeteaseCloudMusicClient(this) : null;
	
	if (this.music != null)
	{
		this.music.addListener('userChanged', mxUtils.bind(this, function()
		{
			this.updateUserElement();
			this.restoreLibraries();
		}))
...
App.prototype.showSplash = function(force)
{
  ...
  if (this.editor.isChromelessView())
	 ...
	 */
		...
		const file = new LocalFile(this, fileObj.data, fileObj.title);
		this.loadFile(urlParams['id'], true, file);
...
App.prototype.saveFile = function(forceDialog, success)
{
	...
	if (file != null)
	{
		...
		{
			console.log('There is saveAs logic: ');
			/**
			 * 如果是 saveAs，生成一下缩略图，列表展示的时候要用到
			 */
			this.getCurrentFileThumbnail(.35);
			/**
			 * 然后是存储逻辑
			 */
			var filename = (file.getTitle() != null) ? file.getTitle() : this.defaultFilename;
			...
			var rowLimit = (serviceCount < 4) ? 3 : (serviceCount > 6 ? 4 : 3);
			...
			var dlg = new CreateDialog(this, filename, mxUtils.bind(this, function(name, mode, input)
			{
				if (name != null && name.length > 0)
				{
					...
					else
					{
						...
						else if (mode === App.MODE_MUSIC) {
							console.log('云音乐的保存，在这里书写: ', this.music);
							this.music.saveFile();
						}
